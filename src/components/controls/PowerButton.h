 ...
 *
 * Renders a rounded-rect background with the IEC 5009 power icon (the
 * "standby" arc-and-line symbol) drawn from an inline SVG.  Colours for
 * on/off states are configurable.
 *
 * The underlying juce::ToggleButton is publicly accessible so you can
 * attach an APVTS ButtonAttachment to it directly.
 */
...
public:
  PowerButton() {
    ...
    // Build the power icon drawable from inline SVG
    rebuildIcon();
  }

  ~PowerButton() override { toggle.setLookAndFeel(nullptr); }
...
private:
  ...
  // ---- Colours ----
  juce::Colour onColour{ScomeotropeColors::toggleOn};
  juce::Colour offColour{ScomeotropeColors::labelText};
  juce::Colour bgColour{ScomeotropeColors::panelBg};
  juce::Colour borderColour{ScomeotropeColors::panelBorder};
